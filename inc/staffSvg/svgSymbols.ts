...
const pathFrameSymbol = (symbol, frame) => elem => {
	if (elem.identity.type === "path" && elem.identity.d.replace(/\d+/g, "") === frame)
		return {symbol};
};
...
const symbolRules = [
	...
	conditionSymbol("STAVES_CONNECTION", elem => elem.identity.type === "rect" && elem.identity.width === 0.25 && elem.identity.height >= 12),
